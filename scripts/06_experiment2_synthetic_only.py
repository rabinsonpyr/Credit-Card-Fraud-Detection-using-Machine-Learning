"""
06_experiment2_synthetic_only.py
=================================
Experiment 2 — Synthetic-Only Training
Trains classifiers EXCLUSIVELY on synthetic data from the best synthesiser.
Evaluates on the SAME REAL HELD-OUT TEST SET from Experiment 1.

NOTE: The synthetic CSV used is determined by 04_synthesiser_quality_check.py.
Update SYNTHETIC_FILE below if the best synthesiser changes.

Sanity checks:
  ✓ Test set row count and fraud count match Experiment 1
  ✓ Synthetic dataset structure and columns
  ✓ No real rows leaked into synthetic training data
...
# Uses row count + fraud count (robust, no floating point issues)
...
print(SEP)
print(f"\nNext step: run 07_experiment3_augmentation.py")
