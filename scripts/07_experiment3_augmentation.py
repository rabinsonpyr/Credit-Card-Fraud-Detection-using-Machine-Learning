"""
07_experiment3_augmentation.py
================================
Experiment 3 — Synthetic Augmentation + Benchmark Conditions
Augments real training data with synthetic fraud cases from the
best synthesiser at five target fraud rates: 1%, 5%, 10%, 25%, 50%.
Also runs benchmark conditions: SMOTE, ADASYN, Cost-Sensitive, RUS.

Sanity checks:
  ✓ Test set row count and fraud count match Experiment 1
  ✓ Synthetic fraud pool is all Class=1 (fraud only)
  ✓ Synthetic pool large enough for all ratios
  ✓ No real rows leaked into synthetic pool
  ✓ Augmented ratio verified at each condition
  ✓ No NaN/Inf in training data

Run order: 02 → 03a/03b/03c → 04 → 05 → 06 → 07 → 08
"""
...
# ── SECTION 2 — Load synthetic fraud pool ────────────────────────────────────
print(f"\nSECTION 2 — LOADING SYNTHETIC FRAUD POOL")
print(SEP)
...
assert (synth_df[TARGET_COL] == 1).all(), \
    f"SANITY FAIL: Fraud pool contains non-fraud rows."
assert synth_df.isnull().sum().sum() == 0, \
    "SANITY FAIL: Fraud pool contains missing values"
print(f"  ✓ Sanity: all {len(synth_df):,} rows confirmed Class=1 (fraud only)")
...
assert len(synth_df) >= max_needed, \
    f"SANITY FAIL: Fraud pool ({len(synth_df):,}) insufficient for {max_ratio*100:.0f}% ratio"
print(f"  ✓ Sanity: pool ({len(synth_df):,}) sufficient for all ratios")
...
# ── SECTION 3A — Augmentation ratio sensitivity ───────────────────────────────
print(f"\nSECTION 3A — AUGMENTATION RATIO SENSITIVITY")
print(SEP)
...
# ── SECTION 3B–3E — Benchmark conditions ─────────────────────────────────────
...
# ── SECTION 4 — Master results ────────────────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 4 — MASTER RESULTS TABLE")
print(SEP)
...
# ── SECTION 5 — Cross-experiment summary ─────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 5 — CROSS-EXPERIMENT BEST PERFORMANCE SUMMARY")
print(SEP)
...
print(f"\n{SEP}")
print("ALL DONE — EXPERIMENT 3")
print(SEP)
print(f"\nNext step: run 08_experiment4_deep_learning.py")
