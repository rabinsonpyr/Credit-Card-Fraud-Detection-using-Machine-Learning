"""
08_experiment4_deep_learning.py
=================================
Experiment 4 — Deep Learning Models
Trains and evaluates four deep learning models for credit card fraud detection:

  Model 1 — VAE (Variational Autoencoder) — unsupervised anomaly detection
    Trained on LEGITIMATE transactions only.
    Fraud detected by high reconstruction error at test time.
    Threshold tuned to maximise F1 on training data.
    No imbalance handling needed (unsupervised).

  Model 2 — TabNet — supervised binary classification
    Attention-based deep learning model designed for tabular data.
    Same 5 imbalance handling conditions as Experiment 1 for fair comparison.

  Model 3 — Tabular ResNet — supervised binary classification
    Feedforward network with residual connections for tabular data.
    Same 5 imbalance handling conditions as Experiment 1.

  Model 4 — TabPFN — supervised binary classification
    Pre-trained transformer, no training needed.
    Subsampled to 10,000 training rows (model design constraint).
    No imbalance handling applied (TabPFN handles this internally).

All models use:
  - Same fixed test set as Experiments 1, 2, 3
  - Same 5 random seeds [42, 123, 456, 789, 1234]
  - Same StandardScaler from Experiment 1
  - Binary F1 (fraud class), AUC-PR, MCC as primary metrics

Installation requirements (run once before this script):
  pip install pytorch-tabnet tabpfn

Sanity checks:
  ✓ Test set row count and fraud count match Experiment 1
  ✓ No NaN/Inf in input data
  ✓ Model output probabilities in valid range [0, 1]

Run order: 02 → 03a/03b/03c → 04 → 05 → 06 → 07 → 08 → 09
"""
...
# TabPFN row limit — model design constraint
...
# ── SECTION 1 — Load data ─────────────────────────────────────────────────────
print(SEP)
print("EXPERIMENT 4 — DEEP LEARNING MODELS")
print(SEP)
...
assert y_test.sum() == EXPECTED_TEST_FRAUD, \
    f"SANITY FAIL: Test set has {y_test.sum()} fraud, expected {EXPECTED_TEST_FRAUD}"
print(f"  ✓ Sanity: matches Experiment 1 ({EXPECTED_TEST_ROWS} rows, {EXPECTED_TEST_FRAUD} fraud)")
...
assert not np.any(np.isnan(X_train)), "SANITY FAIL: NaN in training data"
assert not np.any(np.isnan(X_test)),  "SANITY FAIL: NaN in test data"
print("  ✓ Sanity: no NaN/Inf in data")

# ── MODEL DEFINITIONS ─────────────────────────────────────────────────────────

class VAE(nn.Module):
    """
    Variational Autoencoder for unsupervised fraud detection.
    Trained on LEGITIMATE transactions only.
    Fraud detected by high reconstruction error at test time.

    Architecture:
      Encoder: 31 → 16 → mean(8), logvar(8)
      Decoder: 8  → 16 → 31
...
class TabularResNet(nn.Module):
    """
    Tabular ResNet for supervised fraud classification.
    Uses residual connections to allow gradients to flow through
    deep layers without degradation.

    Architecture:
      Input: 31 features
      Input projection: 31 → 64
      Residual Block 1: 64 → 64 → 64 (with skip connection)
      Residual Block 2: 64 → 64 → 64 (with skip connection)
      Output: 64 → 1 (sigmoid)
    """
...
# ── SECTION 2 — VAE (Unsupervised) ───────────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 2 — MODEL 1: VAE (Variational Autoencoder, Unsupervised)")
print(SEP)
print("""
  Strategy:
    1. Train VAE ONLY on legitimate transactions
    2. VAE learns to compress and reconstruct normal patterns
    3. Unlike standard AE, VAE uses reparameterisation + KL divergence
       to learn a smooth, structured latent space
    4. At test time: high reconstruction error = likely fraud
    5. Threshold tuned to maximise F1 on training reconstruction errors
    NOTE: No imbalance handling — unsupervised approach
""")
...
print("""
  Strategy:
    Attention-based deep learning model purpose-built for tabular data.
    Uses sequential attention to select relevant features at each step.
    Same 5 imbalance handling conditions as Experiment 1.
    Evaluated on same real held-out test set.
""")
...
for cond_key, cond_name in supervised_conditions.items():
    ...
    for seed in SEEDS:
        ...
        assert np.all(y_prob >= 0) and np.all(y_prob <= 1), \
            "SANITY FAIL: TabNet output probabilities outside [0,1]"
...
# ── SECTION 4 — Tabular ResNet ────────────────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 4 — MODEL 3: TABULAR RESNET (Supervised)")
print(SEP)
print("""
  Strategy:
    Feedforward network with residual (skip) connections.
    Residual connections allow gradients to flow through deep
    layers without vanishing — improves training stability.
    Same 5 imbalance handling conditions as Experiment 1.
""")
...
# ── SECTION 5 — TabPFN ────────────────────────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 5 — MODEL 4: TABPFN (Supervised, Pre-trained Transformer)")
print(SEP)
print(f"""
  Strategy:
    Pre-trained transformer trained on thousands of synthetic datasets.
    No training step needed — makes predictions directly.
    Hard row limit: training set subsampled to {TABPFN_MAX_TRAIN_ROWS:,} rows
    Subsampling is stratified to preserve fraud rate.
    No imbalance handling applied — TabPFN handles class imbalance
    internally via its pre-training prior.
    Same 5 seeds used for stratified subsampling reproducibility.
""")
...
for seed in SEEDS:
    ...
    print(f"  ✓ Fraud rate preserved in subsample")
    ...
    assert np.all(y_prob >= 0) and np.all(y_prob <= 1), \
        "SANITY FAIL: TabPFN output probabilities outside [0,1]"
...
# ── SECTION 6 — Master results ────────────────────────────────────────────────
print(f"\n{SEP}")
print("SECTION 6 — MASTER RESULTS TABLE")
print(SEP)
...
# ── SECTION 7 — Comparison with Experiment 1 ─────────────────────────────────
print(f"\n{SEP}")
print("SECTION 7 — COMPARISON WITH EXPERIMENT 1 (Classical Baseline)")
print(SEP)
...
print(f"\n{SEP}")
print("ALL DONE — EXPERIMENT 4 DEEP LEARNING")
print(SEP)
print(f"""
Models trained:
  VAE           — unsupervised, trained on legit only, threshold-based detection
  TabNet        — supervised, attention-based, 5 imbalance conditions
  TabularResNet — supervised, residual connections, 5 imbalance conditions
  TabPFN        — supervised, pre-trained transformer, {TABPFN_MAX_TRAIN_ROWS:,}-row subsample

Files saved:
  {master_path}
  {raw_path}

Next step: run 09_generate_figures.py to produce all thesis figures
  including Experiment 4 comparisons.
""")
